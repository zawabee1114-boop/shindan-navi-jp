// scripts/add-hero-styles.mjs
// TOP ページのスタイル: 新ヘッダー + 新ヒーロー の CSS を旧ヘッダー・旧ヒーロー CSS に置き換え
import { readFileSync, writeFileSync } from 'fs';

const filePath = 'C:/Users/ZAWA/repos/shindan-navi-jp/src/pages/index.astro';
let content = readFileSync(filePath, 'utf-8');

// 旧 .site-header スタイルを新スタイルに置換
const OLD_HEADER_STYLE = `    /* ヘッダー */
    .site-header {
      position: sticky;
      top: 0;
      z-index: 100;
    ...
    .header-cta:hover { background: var(--sn-primary-dark); }`;

const NEW_HEADER_STYLE = `    /* ===== ヘッダー v2.0 ===== */
    .site-header-v2 {
      position: sticky;
      top: 0;
      z-index: 100;
...
if (content.includes(OLD_HEADER_STYLE)) {
  content = content.replace(OLD_HEADER_STYLE, NEW_HEADER_STYLE);
  console.log('header style replaced');
} else {
  console.log('WARNING: old header style not found exactly');
}

// 旧ヒーロースタイルを新スタイルに置換
const OLD_HERO_STYLE_MARKER = '    /* ヒーローセクション */\n    .hero {';
const heroStyleStart = content.indexOf(OLD_HERO_STYLE_MARKER);
// find end of hero style block (next /* */ comment before next class)
const heroStyleEnd = content.indexOf('    /* 3つの理由', heroStyleStart);
console.log('hero style:', heroStyleStart, heroStyleEnd);

const NEW_HERO_STYLE = `    /* ===== ヒーロー v2.0 ===== */
    .hero-v2 {
    ...
    .hero-v2-underline {
      background: linear-gradient(transparent 65%, #ffd9ec 65%);
      padding: 0 2px;
    }
    .hero-v2-desc {
      font-size: 14px;
...
if (heroStyleStart !== -1 && heroStyleEnd !== -1) {
  content = content.slice(0, heroStyleStart) + NEW_HERO_STYLE + content.slice(heroStyleEnd);
  console.log('hero style replaced');
} else {
  // Just insert before existing .reasons-grid or similar
  console.log('WARNING: hero style range not found, appending before </style>');
  content = content.replace('</style>', NEW_HERO_STYLE + '\n    </style>');
}
